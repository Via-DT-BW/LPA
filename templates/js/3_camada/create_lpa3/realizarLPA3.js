function realizarLPA3() {
    ...
    fetch("/get_user_data3")
        .then(response => response.json())
        .then(user => {
            document.getElementById("username").value = user.username || "";
            document.getElementById("selected-line").innerText = selectedLine;
            document.getElementById("dataAtual").value = formatarDataAtual3();
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="btn-group btn-group-toggle mt-2" data-toggle="buttons">
                                    <label class="btn btn-outline-success">
                                        <input type="radio" name="item${index}" id="ok${index}" value="OK" autocomplete="off" onchange="toggleNokFields3(${index})"> OK
                                    </label>
                                    <label class="btn btn-outline-danger">
                                        <input type="radio" name="item${index}" id="nok${index}" value="NOK" autocomplete="off" onchange="toggleNokFields3(${index})"> NOK
                                    </label>
                                    <label class="btn btn-outline-warning">
                                        <input type="radio" name="item${index}" id="nc${index}" value="NC" autocomplete="off" onchange="toggleNokFields3(${index})"> NC
                                    </label>
                                    <label class="btn btn-outline-secondary">
                                        <input type="radio" name="item${index}" id="nt${index}" value="NT" autocomplete="off" onchange="toggleNokFields3(${index})"> NT
                                    </label>
                                </div>
